...
if __name__ == '__main__':
    ...
    # 把变量放到 broadcast 内部， 然后从 broadcast 内部取出来，中间传输逇就是 broadcase 对象
    # 只要中间传输的是 broadcast 对象， saprk就会留意, 只会给每个 executor 发一份
    broadcast = sc.broadcast(stu_info_list)

    def process(data):
        id = data[0]
        name = ''
        for s in broadcast.value:
            if s[0] == id:
                name = s[1]
    ...
    # 2. 假如一个executor( 一个executor就是一个进程 )上运行了2个分区， 每个分区都会收到一份stu_info_list，
    #    但是实际上一个executor上只需要一份就够了， 分区间可以共享stu_info_list
    # 如何解决： 使用广播变量
